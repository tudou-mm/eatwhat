 ...
 *   L2  SDK 加载失败（网络 / 沙箱 / 域名白名单没加）→ 退到**内置坐标库 + 关键词搜索**
 ...
 *
 *   两条出路：① 用 restFallback 直连 REST 拿地址（默认开，立刻可用）；
 *            ② 到控制台新建「Web端(JS API)」Key + 安全密钥，两者都填进配置文件。
 *
  ...
  function reverseGeocode(lng, lat, silent) {
    ...
    function giveUp(code) {
      ...
      if (code === '10009') {
        ...
        hint = '地址解析被拒：这个 Key 在高德控制台注册的平台类型是「Web服务」，'
             + '不是「Web端(JS API)」，所以 JS API 的地址接口不认它（错误码 10009）。'
             + '地图仍可用、经纬度也准确 —— 请手动填写详细地址；'
             + '或到高德控制台新建一个「Web端(JS API)」Key 填进配置。';
      } else if (code === '10008') {
    ...
    loadSdk().then(function (ok) {
      ...
      if (ok) {
        ...
        try {
          ...
          // SDK 在但初始化炸了（Key 域名白名单没加 / 配额用尽 / 容器尺寸为 0）——
          // 这种时候不能白屏，直接退列表。
          // 把真实原因带上：降级提示只写「检查域名白名单」会把人引偏，
          // 实际踩过的是「容器高度还没算出来」。
